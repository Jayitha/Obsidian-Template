...
When using the Periodic Notes plugin, the daily notes get saved to the `Daily Notes` folder. 

You can also use the Periodic Notes plugin to create Daily / Weekly / Monthly / Quarterly / Yearly notes
...
Copies over all incomplete tasks from the last daily note / planner to the current one.  

> [!info]
> When using rollovers, the previous tasks will be deleted to conserve the number of TODOs in the checklist. 
